...
/**
 * AdminAssigneeOrderFlowTest handles a complete end-to-end API flow between Admin and Assignee users.
 *
 * Flow Summary:
 * - Admin logs in and creates an order
 * - Admin splits the order (without creating multiple orders)
 * - Admin updates (TAT) for a specific stage
 * - Admin creates a new user (assignee) and assigns stages 2 to 7
 * - Admin adds remarks and attachments to each stage
 * - The session is switched to the assignee
 * - Assignee accepts terms and conditions
 * - For each assigned stage, assignee:
 *   - Adds a remark
 *   - Uploads an attachment
 *   - Completes the stage
 *   
 * Requirements:
 * - Valid admin and assignee credentials must be configured
 * - APIs for order creation, stage management, user creation, remarks, attachments, and status updates must be functional
 *
 * Test Structure:
 * - Each test method depends on the successful execution of the previous method
 * - If any critical step fails, the remaining dependent steps will be skipped
 *
 * Author: QA@47Billion
 */
...
public class AdminAssigneeOrderFlowTest {

	String adminEmail = "divyaadmin@yopmail.com";
	String adminPassword = "KestrelPro@123";
	
    String customerName = "Mehak Automation";
    ...
    public void termAndConditionAssignedUser() {
    	ExtentTest test = ExtentTestNGListener.getTest();
