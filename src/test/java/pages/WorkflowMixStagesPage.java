...
public class WorkflowMixStagesPage {

	WebDriver driver;

	public WorkflowMixStagesPage(WebDriver driver) {
		this.driver = driver;
	...
	public void WorkflowsAlternateOpenStages(String workflowName) throws InterruptedException {
		Thread.sleep(1000);
		workflowsLink.click();
		Thread.sleep(3000);
		...
		for (int i = 3; i <= 11; i++) {
			...
			if (i % 2 == 0) {
				...
				// Odd stages → Click Automated Days dropdown
	...
	public void verifyWorkflow(String workflowName) throws InterruptedException {
		...
		List<WebElement> workflowElements = driver
				.findElements(By.xpath(String.format("//tbody/tr/td/div/h5[text()='%s']", workflowName)));

		Thread.sleep(2000);
		Assert.assertFalse(workflowElements.isEmpty(),
				"Workflow '" + workflowName + "' should be visible in UI after search");
	}
